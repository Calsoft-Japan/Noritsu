...
codeunit 50002 "ServContractMgmt Copy"
{
            ...
            if FromServiceContractLine."Invoiced to Date" >= FromServiceContractLine."Contract Expiration Date" then begin
                ...
                if CreditAmount > 0 then begin
                    ...
                    CreateAllCreditLines(
                        CreditMemoNo, FromServiceContractLine."Line Amount", WDate, FromServiceContractLine."Invoiced to Date",
                        LineDescription, FromServiceContractLine."Service Item No.", FromServiceContractLine."Item No.",
                        ServContractHeader, FromServiceContractLine."Line Cost", FromServiceContractLine."Line Value",
                        LastIncomePostingDate, FromServiceContractLine."Starting Date");
    ...
    begin
        ...
        if ServLedgEntry.FindLast() then begin
    ...
    begin
            ...
            if ServContractLine.Find('-') then
                repeat
                    ...
                    if ServContractToInvoice.Prepaid then
                        if (ServContractLine."Starting Date" < ServContractToInvoice."Next Invoice Date") and (ServContractLine."Invoiced to Date" = 0D) then begin
                            PartInvoiceFrom := ServContractLine."Starting Date";
                            PartInvoiceTo := ServContractToInvoice."Next Invoice Date" - 1;
                            ServiceApplyEntry :=
                              CreateServiceLedgEntry(
                                ServHeader, ServContractToInvoice."Contract Type", ServContractToInvoice."Contract No.", CalcDate('<-CM>', PartInvoiceFrom), PartInvoiceTo,
                            ...
                            if ShouldCraeteServiceApplyEntry then
                                CreateServiceLine(ServHeader, ServContractToInvoice."Contract Type", ServContractToInvoice."Contract No.", PartInvoiceFrom, PartInvoiceTo, ServiceApplyEntry, false);
                            ServiceApplyEntry := 0;
                        end;
        ...
        ServContractToInvoice.Validate("Last Invoice Date", ServContractToInvoice."Next Invoice Date");
    ...
    begin
        ...
        if (ServiceLedgerEntry."Amount (LCY)" <> 0) or (ServiceLedgerEntry."Discount %" > 0) then
            if ServHeader."Currency Code" <> '' then begin
                ...
                ServLine.Validate("Line Amount",
                  AmountToFCY(TotalServLine."Line Amount", ServHeader) - TotalServLineLCY."Line Amount");
            end else begin
                ServLine.Validate("Unit Price");
                ServLine.Validate("Line Amount", -ServiceLedgerEntry."Amount (LCY)");
            end;
